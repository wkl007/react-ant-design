import React, { Component, FC, Fragment, ReactNode } from 'react'
...
const { yuan } = Charts
const ChartCard: any = Charts.ChartCard
const MiniBar: any = Charts.MiniBar
const MiniProgress: any = Charts.MiniProgress
const MiniArea: any = Charts.MiniArea
const Field: any = Charts.Field
const Bar: any = Charts.Bar
const Pie: any = Charts.Pie
const TimelineChart: any = Charts.TimelineChart

const { TabPane } = Tabs
const { RangePicker } = DatePicker

const rankingListData: Array<any> = []
for (let i = 0; i < 7; i += 1) {
...
const Yuan = (props: { children: string | number; }) => (
...
class Analysis extends Component<AnalysisProps, AnalysisState> {
  state = {
    salesType: 'all',
    currentTabKey: '',
    rangePickerValue: getTimeDistance(dateType.year),
    chart: chartData,
    loading: true
  }
  ...
  render () {
    const { salesType, currentTabKey, rangePickerValue, chart, loading } = this.state
    const {
      visitData,
      visitData2,
      salesData,
    ...
    const salesExtra: ReactNode = (
      <div className={styles.salesExtraWrap}>
        <div className={styles.salesExtra}>
          <span
            className={classNames(this.isActive(dateType.today), 'link-button')}
            onClick={() => this.selectDate(dateType.today)}>
            今日
          </span>
          <span
            className={classNames(this.isActive(dateType.week), 'link-button')}
            onClick={() => this.selectDate(dateType.week)}>
            本周
          </span>
          <span
            className={classNames(this.isActive(dateType.month), 'link-button')}
            onClick={() => this.selectDate(dateType.month)}>
            本月
          </span>
          <span
            className={classNames(this.isActive(dateType.year), 'link-button')}
            onClick={() => this.selectDate(dateType.year)}>
            全年
          </span>
        </div>
        <RangePicker
          ...
          <Col xl={12} lg={24} sm={24} xs={24}>
            ...
            >
              ...
              <Pie
                ...
                data={salesPieData}
                valueFormat={(value: string | number) => <Yuan>{value}</Yuan>}
                height={248}
                lineWidth={4}
              />
            </Card>
